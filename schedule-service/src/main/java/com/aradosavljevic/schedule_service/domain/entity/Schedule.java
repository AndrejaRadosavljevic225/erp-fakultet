...
public class Schedule extends Raspored {
    public Schedule(List<Prostorija> prostorije, List<Termin> termini, LocalDate vaziOd, LocalDate vaziDo) {
        super(prostorije, termini,vaziOd,vaziDo);

    }
    public Schedule(List<Prostorija> prostorije, List<Termin> termini) {
    ...
    public boolean brisanjeTermina(Prostorija prostorija, LocalTime vreme, LocalDate datum) {
        ...
        for(Termin t: termini){
            if(t.getProstorija().getNaziv().matches(prostorija.getNaziv())
                ...
                x = x.plusDays(7);
                if(x.compareTo(datum)==0){
                    if(((Reservation)t).getNumber()==1){
                        temp.add(t);
                    }else{
                        ((Reservation)t).setNumber((((Reservation) t).getNumber()-1));
                    }
                }
        ...
        for(Termin t: termini) {
            if(((Reservation)t).getNumber()<1)temp.add(t);
        }
            ...
            for (ConfigMaper entry : columnMapiranja) {
                ...
                switch (mappings.get(columnIndex)) {
                    ...
                    case "datum":
                        ...
                        break;
                    case "brojTermina":
                        appointment.setNumber(Long.parseLong(record.get(columnIndex)));
                        break;
                    case "dan":
    ...
    private void ispisiApache(String path) throws IOException {
        ...
        for (Termin appointment : super.getTermini()) {
            csvPrinter.printRecord(
                    ...
                    ((Reservation)appointment).getNumber(),
